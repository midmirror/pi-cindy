...
### 48. 跨进程 refresh 互斥落地：同 pid 覆盖条件会让同进程并发全进临界区

背景：EXPERIENCE #23/#46 遗留——多 pi 进程 / 热重载叠加时并发 refresh 同一 refresh token，
服务端**轮换式**竞争（先到者轮换，后到者用旧 token 被拒）→ 401 → 误清会话。

方案（`src/auth/refresh-lock.ts`）：SQLite 单行锁 `refresh_lock` + 进程内 promise-chain 串行，
双层互斥。锁内**重读 session.enc**（可能已被他进程轮换），绝不用进锁前陈旧 token；持锁进程
崩溃靠 `locked_at` 过期（30s）由后续进程抢占。

**踩坑**：初版 GRAB 用 `owner_pid = ?`（同 pid）作覆盖条件——本想服务热重载幽灵锁（同 pid 旧
实例残留立即接管），但同进程内所有调用 pid 相同，**并发调用全部互相覆盖进临界区**（测试当场
抓到最大并发=5）。教训：**pid 不是「实例身份」，同进程多实例/多调用共享 pid**——用 pid 判
覆盖 = 同进程互斥失效。修复：进程内 promise-chain 串行兜底（同进程严格排队），pid 覆盖条件
仅保留给热重载幽灵锁恢复（此时同进程只有一个活调用）。跨进程互斥由 SQLite 锁保证，同 pid
覆盖在串行下不会误伤。
